...
def create_model_and_save(data_dir, batch_size, epochs_number, model_dst, model_name='MyModel'):
    ...
    CoAtNet0 = coatnet_0(len(class_names))
    # netCoAtNet6 = CoAtNet6(num_classes=len(class_names))
    # netCoAtNet7 = CoAtNet7(num_classes=len(class_names))

    # Define a Loss function and optimizer
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.SGD(CoAtNet0.parameters(), lr=0.001, momentum=0.9)

    model, total_accuracy, total_loss = train_network(CoAtNet0, epochs_number, dataset, dataloader, criterion, optimizer)
    # model, total_accuracy, total_loss = train_network(netCoAtNet6, epochs_number, dataset, dataloader, criterion, optimizer)
    # model, total_accuracy, total_loss = train_network(netCoAtNet7, epochs_number, dataset, dataloader, criterion, optimizer)
    # model, total_accuracy, total_loss = None, None, None
...
def create_plot(model_dst, camera_position, model_name='MyModel'):
...
def test_model_common_accuracy(data_dir, model_dst='./models', camera_position='Forward', model_name='MyModel'):
    data_transforms = {
        'train': transforms.Compose([
            transforms.RandomHorizontalFlip(),
            transforms.Resize((224, 224)),
    ...
    # Define a Convolutional Neural Network
    net = coatnet_0(len(class_names))

    # for camera_position in camera_positions:
    model_file = gen_model_name(model_name='CoAtNet0', camera_position=camera_position)
...
if __name__ == '__main__':
    ...
    create_model_and_save(data_dir, batch_size, epochs_number, model_dst, model_name=model_name)

    test_model_common_accuracy(test_dir, model_dst, camera, model_name=model_name)
    test_model_separate_accuracy(test_dir, model_dst, camera, model_name=model_name)

    create_plot(model_dst, camera, model_name=model_name)
