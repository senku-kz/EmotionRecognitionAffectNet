...
def training_the_model(model, dataset, dataloader, class_names, epoch_num=1, lr=5e-4):
    ...
    for epoch in range(epoch_num):  # loop over the dataset multiple times
        ...
        train_loss, train_acc = train_one_step(model, dataset['test'], dataloader['test'], optimizer, criterion, device)
        valid_loss, valid_acc = evaluate(model, dataset['val'], dataloader['val'], criterion, device)
...
def train_one_step(model, dataset, dataloader, optimizer, criterion, device):
    epoch_loss = 0
    epoch_acc = 0
...
def evaluate(model, dataset, iterator, criterion, device):
    epoch_loss = 0
    epoch_acc = 0
    sample_size = len(dataset)
    # model.eval()
...
def test_model_separate_accuracy(model, batch_size, data_dir, model_dst='./models_trained'):
    mini_batches = 50
    data_transforms = {
        'train': transforms.Compose([
            transforms.RandomHorizontalFlip(),
    ...
    # model = coatnet_0(len(class_names))
